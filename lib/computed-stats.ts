...
export function outPercentage(innings: number, notOuts: number): number {
...
export function conversionRate(fifties: number, timesReached30: number): number {
...
export function boundaryPercentage(fours: number, sixes: number, totalRuns: number): number {
...
export type FormLevel = "Excellent" | "Good" | "Average" | "Poor" | "Unknown"

export function formRating(lastFiveScores: number[]): {
    level: FormLevel
    color: string
    avg: number
} {
...
interface ImpactInputs {
    ...
    leagueAvgSR: number
    leagueAvgAvg: number
    leagueAvgBoundaryPct: number
}

export function impactScore(inputs: ImpactInputs): number {
    const {
...
export type AuctionValueLabel = "Good Value" | "Fair Value" | "Overpriced"

export function auctionValueRating(
    playerImpactScore: number,
    auctionPrice: number,
    leagueAvgPrice: number
): { label: AuctionValueLabel; ratio: number } {
...
export type PhaseLevel = "Elite" | "Good" | "Average" | "Poor"

export function phaseRating(playerSR: number, leagueAvgSR: number): {
    level: PhaseLevel
    color: string
    delta: number
} {
...
interface BatterRatingInputs {
    avg: number        // raw batting average
    sr: number         // strike rate
    ppSRvsAvg: number  // powerplay SR delta vs league (ratio)
    deathSRvsAvg: number // death SR delta vs league (ratio)
    impact: number     // impact score (0-10)
    fielding: number   // fielding rating (0-10)
}

interface BowlerRatingInputs {
    econVsAvg: number  // economy vs league avg (ratio, lower = better)
    sr: number         // bowling SR
    deathEconVsAvg: number
    wicketsPerMatch: number
    dotBallPct: number // dot ball percentage
}

export function batterRating(inputs: BatterRatingInputs): number {
...
export function bowlerRating(inputs: BowlerRatingInputs): number {
...
export function allRounderRating(batRating: number, bowlRating: number, fielding: number, impact: number): number {
...
export function injuryRiskRating(matchesMissed3Yrs: number, age: number, hasInjuryHistory: boolean): {
    risk: InjuryRisk
    color: string
} {
    let level: InjuryRisk = "Low"
    ...
    const colors: Record<InjuryRisk, string> = {
        Low: "#10b981",
        Medium: "#f59e0b",
        High: "#ef4444",
    }
...
export function pressureIndex(dotBallPct: number, requiredRR: number, wicketsDown: number): number {
...
export function clutchScore(runsInLast3Over: number, wicketsInLast3Over: number, matchResult: "Won" | "Lost"): number {
...
export function normalizeRadarStats(batting: any, bowling: any, impact: number, clutch: number, pressure: number) {
