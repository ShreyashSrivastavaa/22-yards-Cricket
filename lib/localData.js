...
export function getAuctionPlayers() {
  return playersData.filter(p => p.isAuctionTarget)
}

export function getTeamsWithSquads() {
  return teamsData.map(team => ({
    ...team,
    players: playersData.filter(p => p.team?.toLowerCase() === team.name.toLowerCase())
  }))
}

export function getMatchDetails(id) {
  return matchesData.find(m => m.id === id) || null
}
