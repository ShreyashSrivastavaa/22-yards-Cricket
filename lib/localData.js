...
export function getMatchDetails(matchId) {
    ...
    for (const s of seasons) {
        const d = getIPLDeliveriesBySeason(s)
        const matchBalls = d.filter(b => String(b.match_id || "").trim() === searchId)
        if (matchBalls.length > 0) {
            deliveries = matchBalls
            break
        }
    }
    ...
    const matchInfo = deliveries[0]
    const inningsScores = {}

    deliveries.forEach(d => {
        const innNum = String(d.innings || "1")
        if (!inningsScores[innNum]) {
            inningsScores[innNum] = {
                team: d.batting_team,
                runs: 0,
                wickets: 0,
                overs: 0,
        ...
        const inn = inningsScores[innNum]
        const runs = Number(d.runs_off_bat ?? 0)
        const extras = Number(d.extras ?? 0)
        inn.runs += runs + extras
        
        // Count ball for overs (excluding wides/noballs from over count in standard cricket, but here for simplicity we count all legal deliveries)
        // Standard: extras like wides/noballs don't count towards the 6 balls.
        const isWide = Number(d.wides || 0) > 0
        const isNoBall = Number(d.noballs || 0) > 0
        if (!isWide && !isNoBall) {
            inn.balls++
        }
        
        // Batting stats
        const striker = d.striker || "Unknown"
        if (!inn.scorecard.batting[striker]) {
            inn.scorecard.batting[striker] = { name: striker, runs: 0, balls: 0, fours: 0, sixes: 0, out: 'Not Out' }
        }
        const b = inn.scorecard.batting[striker]
        b.runs += runs
        if (!isWide) b.balls++ // Wides don't count as balls faced
        if (runs === 4) b.fours++
        if (runs === 6) b.sixes++

        // Wickets
        const dismissed = d.player_dismissed || ""
        if (dismissed && dismissed !== "" && dismissed !== "NA") {
            inn.wickets++
            if (inn.scorecard.batting[dismissed]) {
                inn.scorecard.batting[dismissed].out = `c ${d.fielder || ""} b ${d.bowler}`
            } else {
                // If player dismissed is not the striker (e.g. run out non-striker)
                inn.scorecard.batting[dismissed] = { name: dismissed, runs: 0, balls: 0, fours: 0, sixes: 0, out: 'Run Out' }
            }
        }

        // Bowling stats
        const bowler = d.bowler || "Unknown"
        if (!inn.scorecard.bowling[bowler]) {
            inn.scorecard.bowling[bowler] = { name: bowler, runs: 0, wickets: 0, balls: 0, legalBalls: 0, economy: "0.0" }
        }
        const bowl = inn.scorecard.bowling[bowler]
        bowl.runs += runs + Number(d.wides || 0) + Number(d.noballs || 0)
        if (!isWide && !isNoBall) {
            bowl.legalBalls++
        }
        bowl.balls++
        if (dismissed && !['run out', 'retired hurt'].includes((d.wicket_type || "").toLowerCase())) {
            bowl.wickets++
        }
    })

    // Compute overs and economy
    Object.values(inningsScores).forEach(inn => {
        inn.overs = `${Math.floor(inn.balls / 6)}.${inn.balls % 6}`
        Object.values(inn.scorecard.bowling).forEach(b => {
            b.overs = `${Math.floor(b.legalBalls / 6)}.${b.legalBalls % 6}`
            const totalOvers = b.legalBalls / 6
            b.economy = totalOvers > 0 ? (b.runs / totalOvers).toFixed(2) : (b.runs * 6).toFixed(2)
        })
    })
    ...
    return {
        ...
        team1: matchInfo.batting_team,
        team2: matchInfo.bowling_team,
        innings: Object.values(inningsScores)
    }
}

/**
 * Get all teams with their full squads grouped.
 */
export function getTeamsWithSquads() {
    const players = getAllPlayers()
    const teams = getIPLTeams()
    
    return teams.map(t => ({
        ...t,
        players: players.filter(p => p.teamCode === t.code || p.teamName === t.name)
    }))
}
